<?php
	if (isset($_POST['key'])) {
		...
		if ($_POST['key'] == 'getRowData') {
			...
			$sql = $conn->query("SELECT namaPelatihan, bidang, noSurat, tglAwal, tglAkhir, klasifikasi, lokasi, vendor, jmlPeserta, durasi, managerial, sertifikasi, fungsional, biaya FROM pelatihan2018 WHERE idPelatihan='$rowID'");
		...
		if ($_POST['key'] == 'getExistingData') {
			...
			$sql = $conn->query("SELECT idPelatihan, namaPelatihan, bidang, noSurat, tglAwal, tglAkhir, klasifikasi, lokasi, vendor, jmlPeserta, durasi, managerial, sertifikasi, fungsional, biaya FROM pelatihan2018 LIMIT $start, $limit");
			if ($sql->num_rows > 0) {
				$response = "";
		...
		if ($_POST['key'] == 'deleteRow') {
			$conn->query("DELETE FROM pelatihan2018 WHERE idPelatihan='$rowID'");
		...
		if ($_POST['key'] == 'updateRow') {
			$conn->query("UPDATE pelatihan2018 SET namaPelatihan='$name', bidang='$bidang', noSurat='$noSurat', tglAwal='$tglAwal', tglAkhir='$tglAkhir', klasifikasi='$klasifikasi', lokasi='$lokasi', vendor='$vendor', jmlPeserta='$jmlPeserta', durasi='$durasi', managerial='$managerial', sertifikasi='$sertifikasi', fungsional='$fungsional', biaya='$biaya' WHERE idPelatihan='$rowID'");
			exit('success');
		}

		if ($_POST['key'] == 'addNew') {
			$sql = $conn->query("SELECT idPelatihan FROM pelatihan2019 WHERE namaPelatihan = '$name'");
				$conn->query("INSERT INTO pelatihan2018 (namaPelatihan, bidang, noSurat, tglAwal, tglAkhir, klasifikasi, lokasi, vendor, jmlPeserta, durasi, managerial, sertifikasi, fungsional, biaya) 
