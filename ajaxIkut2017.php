<?php
	if (isset($_POST['key'])) {
		...
		if ($_POST['key'] == 'getRowData') {
			...
			$sql = $conn->query("SELECT nama, nipp, lokasi, substansi, kelasJabatan, jenisKelamin, idPelatihan, namaPelatihan, durasi, bidang FROM keikutsertaan2017 WHERE no='$rowID'");
		...
		if ($_POST['key'] == 'getExistingData') {
			...
			$sql = $conn->query("SELECT nama, nipp, lokasi, substansi, kelasJabatan, jenisKelamin, idPelatihan, namaPelatihan, durasi, bidang FROM keikutsertaan2017 LIMIT $start, $limit");
			if ($sql->num_rows > 0) {
				$response = "";
		...
		if ($_POST['key'] == 'deleteRow') {
			$conn->query("DELETE FROM keikutsertaan2017 WHERE no='$rowID'");
		...
		if ($_POST['key'] == 'updateRow') {
			$conn->query("UPDATE keikutsertaan2017 SET nama='$nama', nipp='$nipp', lokasi='$lokasi', substansi='$substansi', kelasJabatan='$kelasJabatan', jenisKelamin='$jenisKelamin', idPelatihan='$idPelatihan', namaPelatihan='$namaPelatihan', durasi='$durasi', bidang='$bidang'  WHERE no='$rowID'");
			exit('success');
		}

		if ($_POST['key'] == 'addNew') {
			$sql = $conn->query("SELECT no FROM keikutsertaan2017 WHERE nama = '$nama'");
				$conn->query("INSERT INTO keikutsertaan2017 (nama, nipp, lokasi, substansi, kelasJabatan, jenisKelamin, idPelatihan, namaPelatihan, durasi, bidang) 
