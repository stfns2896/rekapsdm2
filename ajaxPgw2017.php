<?php
    if (isset($_POST['key'])) {
        ...
        if ($_POST['key'] == 'getRowData') {
            ...
            $sql = $conn->query("SELECT nama, nipp, lokasi, substansi, kelasJabatan, jenisKelamin FROM pegawai2017 WHERE no='$rowID'");
        ...
        if ($_POST['key'] == 'getExistingData') {
            ...
            $sql = $conn->query("SELECT nama, nipp, lokasi, substansi, kelasJabatan, jenisKelamin FROM pegawai2017 LIMIT $start, $limit");
			// var_dump($sql);
			if ($sql->num_rows > 0) {
                $response = "";
        ...
        if ($_POST['key'] == 'deleteRow') {
            $conn->query("DELETE FROM pegawai2017 WHERE no='$rowID'");
        ...
        if ($_POST['key'] == 'updateRow') {
            $conn->query("UPDATE pegawai2017 SET nama='$nama', nipp='$nipp', lokasi='$lokasi', substansi='$substansi', kelasJabatan='$kelasJabatan', jenisKelamin='$jenisKelamin' WHERE no='$rowID'");
            exit('success');
        }

        if ($_POST['key'] == 'addNew') {
            $sql = $conn->query("SELECT no FROM pegawai2017 WHERE nama = '$nama'");
            $conn->query("INSERT INTO pegawai2017 (nama, nipp, lokasi, substansi, kelasJabatan, jenisKelamin) 
