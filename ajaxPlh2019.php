<?php
	if (isset($_POST['key'])) {
		...
		if ($_POST['key'] == 'getRowData') {
			...
			$sql = $conn->query("SELECT namaPelatihan, bidang, noSurat, tglAwal, tglAkhir, klasifikasi, lokasi, vendor, jmlPeserta, durasi, managerial, sertifikasi, fungsional, biaya FROM pelatihan2019 WHERE idPelatihan='$rowID'");
		...
		if ($_POST['key'] == 'getExistingData') {
			...
			$limit = $conn->real_escape_string($_POST['limit']);
 			$sql = $conn->query("SELECT idPelatihan, namaPelatihan, bidang, noSurat, tglAwal, tglAkhir, klasifikasi, lokasi, vendor, jmlPeserta, durasi, managerial, sertifikasi, fungsional, biaya FROM pelatihan2019 LIMIT $start, $limit");
			if ($sql->num_rows > 0) {
				$response = "";
		...
		if ($_POST['key'] == 'deleteRow') {
			$conn->query("DELETE FROM pelatihan2019 WHERE idPelatihan='$rowID'");
